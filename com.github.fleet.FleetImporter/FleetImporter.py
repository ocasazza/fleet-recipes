        ...
        try:
            ...
            with urllib.request.urlopen(
                ...
                if resp.getcode() == 200:
                    ...
                    # Debug: show what we got from the search
                    self.output(f"DEBUG: Search for '{base_name}' returned {len(software_titles)} titles")
                    if software_titles:
                        self.output(f"DEBUG: First 3 titles: {[t.get('name') for t in software_titles[:3]]}")
    ...
    def _fleet_upload_package(
        ...
        try:
            ...
            if e.code == 409:
                ...
                self.output(f"DEBUG: 409 error body: {error_body[:200]}")
                ...
                self.output(f"DEBUG: Regex match result: {installer_match}")
                if installer_match:
                    installer_name = installer_match.group(1)
                    self.output(f"Found existing installer: {installer_name}")
