        ...
        try:
            ...
            if e.code == 409:
                ...
                if installer_match:
                    ...
                    # IMPORTANT: Search by software_title, not installer_name, to avoid cross-package matching
                    # when different packages share the same git ref (e.g., "0-41b0d35" could match "1.1.0-41b0d35")
                    try:
                        query_param = urllib.parse.quote(software_title)
                        ...
                        with urllib.request.urlopen(
                            ...
                            if resp_search.getcode() == 200:
                                ...
                                self.output(f"DEBUG: Fleet search for '{software_title}' returned {len(software_titles)} result(s)")
                                for idx, title in enumerate(software_titles):
                                    self.output(f"DEBUG: Result {idx+1}: name='{title.get('name')}', id={title.get('id')}")

                                # Find exact match by software_title (not installer_name)
                                for title in software_titles:
