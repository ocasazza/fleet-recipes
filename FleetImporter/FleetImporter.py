...
""".strip()
    ...
    def _fleet_upload_package(
        ...
        try:
            ...
            if e.code == 409:
                # Package already exists - delete and retry (unless already retrying)
                self.output(
                    f"Package already exists in Fleet (409 Conflict). Deleting and retrying..."
                )
                ...
                if self._fleet_delete_package(base_url, token, software_title, team_id):
                    ...
                    raise ProcessorError("Failed to delete existing package, cannot retry upload")
